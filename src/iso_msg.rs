...
impl<'a, 'b> IsoMsg<'a, 'b> {
    ...
    pub fn remove_field(&mut self, index: usize) -> anyhow::Result<()> {
    ...
    pub fn set_field(&mut self, index: usize, buffer: &[u8]) -> anyhow::Result<()> {
        trace!(
    ...
    fn get_field_raw(&self, index: usize, buffer: &mut [u8]) -> anyhow::Result<(usize, usize)> {
        assert!(index < self.fields.len());
        let field = &self.fields[index];
        if !field.exist {
            return Err(anyhow::anyhow!("Field not set"));
        }
        ...
        if field.new_payload.is_some() {
            ...
            return Err(anyhow::anyhow!("Input buffer is smaller than field value"));
        }
        if field.len == 0 {
            return Err(anyhow::anyhow!("Field not set"));
        }
        if buffer.len() >= field.len && self.payload.len() >= (field.len + field.index) {
            ...
            Err(anyhow::anyhow!("Input buffer is smaller than field value"))
        }
    }
...
mod tests {
    ...
    fn init_iso_msg_test() {
        ...
        }
        {
            ...
            // assert_eq!(res, Err(anyhow::anyhow!("Field not set")));
        }
        {
